 ...
 *
 * La lista è definita come un puntatore opaco a una struttura.
 * Per inizializzare una lista, si deve usare TNCList_new().
 *
 * Gli elementi della lista sono rappresentati internamente come puntatori a void*,
 * per cui è possibile aggiungere alla lista elementi di ogni tipo (eccetto funzioni) e di diverso tipo tra loro.
 *
 * Gli elementi non vengono copiati prima dell'aggiunta alla lista.
 *
 * Le funzioni di rimozione prendono come parametro un puntatore a una funzione. La funzione viene chiamata
 * sull'elemento da rimuovere per eseguire operazioni di pulizia.
 */
 ...
 *
 * Puntatore a una struttura usata internamente per tenere traccia della posizione del nodo.
 * Elementi di questo tipo vengono restituiti dalle funzioni TNCList_next(), TNCList_previous(), TNCList_first()
 * e TNCList_last(), e possono essere usati per iterare attraverso la lista, per indicare un nodo da rimuovere,
 * o come punto di riferimento per inserire un nuovo valore.
 * Si può ottenere l'elemento contenuto nel nodo con la funzione TNCList_getvalue().
 *
 * @see TNCList_insert_before()
 * @see TNCList_insert_after()
 * @see TNCList_remove()
 *
 */
 ...
 *
 * @return Un puntatore a una lista inizializzata se l'allocazione va a buon fine,
 *         NULL altrimenti.
 */
TNCList TNCList_new();
 ...
 *
 * @param list La lista su cui si vuole operare.
 *             Il comportamento della funzione non è definito se questo parametro è NULL.
 *
 * @return Un TNCListNode corrispondente al primo nodo della lista,
 ...
 * @paramnotnull
 *
 * @return Un TNCListNode corrispondente all'ultimo nodo della lista,
 *         NULL se la lista è vuota.
 *
 * @see TNCListNode
 */
 ...
 *
 * @param current Il nodo di cui si vuole conoscere l'elemento successivo.
 *                Il parametro deve essere un nodo valido di una lista (non deve essere NULL e non deve essere un nodo
 *                che è stato rimosso).
 *
 * @return Il nodo della lista successivo a quello passato come parametro.
 *         NULL se current è l'ultimo nodo della lista.
 *
 * @see TNCListNode
 */
 ...
 *
 * @param current Il nodo di cui si vuole conoscere l'elemento precedente.
 *                Il parametro deve essere un nodo valido di una lista (non deve essere NULL e non deve essere un nodo
 *                che è stato rimosso).
 *
 * @return Il nodo della lista successivo a quello passato come parametro.
 *         NULL se current è il primo nodo della lista.
 *
 * @see TNCListNode
 */
 ...
 *
 * @param ref Il nodo di riferimento.
 *            Il comportamento della funzione non è definito se questo parametro è NULL.
 *
 * @param val Elemento da inserire nella lista
 *
 ...
 *
 * @param node Il nodo corrispondente all'elemento da togliere.
 *             Se NULL, il comportamento della funzione non è definito.
 *
 * @param freefn Una funzione da chiamare per dellocare l'elemento.
 *               Se NULL, non viene chiamata nessuna funzione sull'elemento e questo non viene deallocato.
 *
 */
 ...
 * @param self La lista da cancellare
 *
 * @param freefn Un puntatore a una funzione di deallocazione da chiamare per ogni elemento nella lista.
 *               Se NULL, nessuna funzione verrà chiamata e gli elementi non verranno deallocati.
 *
 *
 */
 ...
 * Concatena due liste.
 *
 * @param list La lista con i primi elementi. Alla conclusione della funzione, questa lista conterrà le due liste
 *             concatenate. Questo parametro non deve essere NULL.
 *
 * @param to_chain La seconda lista da concatenare. Questa lista viene distrutta al termine della funzione.
